...
常量池表中的地址可能对应着一个类名、方法名、参数类型等信息。具体信息看代码。**就是一张表**
...
速度较快，但是会产生内存碎片（灰色是垃圾）

<img src="img/20210209142921425.png" alt="20210209142921425" style="zoom:50%;" />

当有一个比较大的对象要放入堆的时候，这些内存碎片总和能够放得下，但每一个碎片都不能放得下

## 标记整理（Mark Compact）

速度慢（移动的时候，对象的地址会发生改变，地址一发生改变，运行的程序中局部变量（指针罢了）或者其他引用这些被移动对象的变量（指针指向的地址）的地址都要变，所以速度慢），但是没有内存碎片<img src="img/20210209143504936.png" alt="20210209143504936" style="zoom:50%;" />
...
<img src="img/20210209144026784.png" alt="20210209144026784" style="zoom:50%;" />

把from生存对象放入to中，交换from和to，to又成为下一个from。复制方式应该也有标记整理的确定，但是应该会比标记整理快一点，他不用考虑地址前进多少的问题

**虚拟机是结合上述三种去做垃圾回收算法**

## 分代垃圾回收算法（结合上述三种垃圾回收算法）

因为垃圾回收时如果扫描所有的堆太费时间，所以长时间存活的对象就频率少一点扫描，短的就频率高一点扫描。

首先将堆分为两个大区，一个**新生代，一个老年代**

新生代又分为**伊甸园、幸存区（幸存区From、幸存区To）**

**新生代的垃圾回收器叫做Minor GC**

<img src="img/20210209161407621.png" alt="20210209161407621" style="zoom:50%;" />

新创建的对象在eden区（伊甸园区）

当eden区满的时候，就会触发新生代专属的MinorGC，MinorGC找eden区和幸存区From中的幸存对象，放入幸存区To中，交换From和To，幸存对象寿命+1，其他区域清零（复制算法）

minor gc 会引发 stop the world，暂停其他线程，等垃圾回收结束后，恢复用户线程运行

当幸存区对象的寿命超过阈值时，会晋升到老年代，最大的寿命是 15（4bit）。当幸存区的空间不足的时候也会提前晋升老年代

当老年代空间不足时，会先触发 minor gc，如果空间仍然不足，那么就触发 full fc ，停止的时间更长！老年代中存活的对象较多，整理起来很耗时间
...
**了解了calss文件的组成，知道怎么个流程就行了，好好练技术，告别底层先**